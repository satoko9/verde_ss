...
void Right(){
  static int right = 0;
    ...
    if (right != PS3.getButtonPress(RIGHT)) {
      right = PS3.getButtonPress(RIGHT);
...
void Down(){
  static int down = 0;
    ...
    if (down != PS3.getButtonPress(DOWN)) {
...
void Left(){
  static int left = 0;
    ...
    if (left != PS3.getButtonPress(LEFT)) {
      left = PS3.getButtonPress(LEFT);
      if (left != PS3.getButtonPress(LEFT) > 0){
        Serial.print("f1#");
      }else{
        Serial.print("f0#");
      }
...
void pitch(){
  static int pitch = 0;
    if (pitch != PS3.getAngle(Pitch)) {
      pitch = PS3.getAngle(Pitch);
      if(PS3.getAngle(Pitch) > 0){
        Serial.write('m');
...
void roll(){
  static int roll = 0;
  if (roll != PS3.getAngle(Roll)) {
    roll = PS3.getAngle(Roll);
    if(PS3.getAngle(Roll) > 0){
      Serial.write('n');
