...
//    roll();
  ...
  if(point == 1){
    for(i = 0; i <= y; i++){
      for(x = 0; x < 8; x++){
        s_data[x] = send_data[x][y];
      }
      CAN0.sendMsgBuf(ID, 0, 8, s_data);
    }
    x = 0, y = 0;
  }
